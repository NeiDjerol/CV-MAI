...
# Проверка аргументов
if len(sys.argv) < 2:
    print("Использование: python script.py <путь_к_видео>")
    sys.exit(1)
...
def calculate_angle_from_points(points):
    """Расчет угла наклона по точкам QR кода"""
    ...
    if avg_width > 0 and avg_height > 0:
        # Чем больше разница между шириной и высотой, тем больше угол
        ratio = min(avg_width, avg_height) / max(avg_width, avg_height)
        # Преобразуем отношение в угол (0-90 градусов)
        angle = (1 - ratio) * 90
        
        # Дополнительно учитываем искажение (разница между противоположными сторонами)
...
while True:
    ...
    if data and points is not None:
        ...
        cv2.putText(display, "QR DECODED", (10, 30), 
                   cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 255, 0), 2)
        cv2.putText(display, f"Angle: {current_angle:.1f} deg", (10, 60), 
        ...
        cv2.putText(display, "QR FOUND but NOT DECODED", (10, 30), 
        ...
        if width > 10 and height > 10:
            ...
            if warped is not None:
                ...
                if data_corr:
                    ...
                    cv2.putText(display, "Correction: SUCCESS", (10, 90), 
                    ...
                    cv2.putText(display, "Correction: FAILED", (10, 90), 
        ...
        cv2.putText(display, "NO QR CODE", (10, 30), 
                   cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
        cv2.putText(display, f"Last angle: {last_angle:.1f} deg", (10, 60), 
    ...
    # Показываем видео
    cv2.imshow('QR Code Detection', display)
